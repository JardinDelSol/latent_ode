...
parser.add_argument(
    "--extraprnn", action="store_true", help="Set extrapolation mode for ODE-RNN"
)
    ...
    for itr in range(1, num_batches * (args.niters + 1)):
        ...
        if itr % (n_iters_to_viz * num_batches) == 0:
            with torch.no_grad():
                ...
                logger.info(
                    "Train loss (one batch): {}".format(train_res["loss"].detach())
                )
                logger.info(
                    "Train CE loss (one batch): {}".format(
                ...
                if "accuracy" in train_res:
                    logger.info(
                        "Classification accuracy (TRAIN): {:.4f}".format(
                            train_res["accuracy"]
                        )
                    )
                ...
                if "accuracy" in test_res:
                    logger.info(
                        "Classification accuracy (TEST): {:.4f}".format(
                            test_res["accuracy"]
                        )
                    )
                ...
                if "pois_likelihood" in test_res:
                    logger.info(
                        "Poisson likelihood: {}".format(test_res["pois_likelihood"])
                    )
            ...
            if args.viz:
                with torch.no_grad():
                    ...
                    print("plotting....")
                    if (
                        isinstance(model, LatentODE) or isinstance(model, ODE_RNN)
                    ) and (
