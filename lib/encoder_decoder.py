...
class Encoder_z0_ODE_RNN(nn.Module):
    ...
    def __init__(
        ...
        self.transform_z0 = nn.Sequential(
            nn.Linear(latent_dim * 2, 100), nn.Tanh(), nn.Linear(100, self.z0_dim * 2),
        )
    ...
    def run_odernn(
        self, data, time_steps, run_backwards=True, save_info=False, test=False
    ):
        # IMPORTANT: assumes that 'data' already has mask concatenated to it \
        ...
        for i in time_points_iter:
            ...
            # print("intrapolation yi_ode", yi_ode.shape)
            # print("interpolation xi shape:", xi.shape)
        ...
        return yi, yi_std, latent_ys, extra_info, t_i
    ...
    def extrap_odernn(
        ...
        for i in time_points_iter:
            ...
            xi = decoder(yi_ode)
            # print("yi_ode shape: ", yi_ode.shape)
            # print("decoded xi shape:", xi.shape)
            xi_mask = torch.zeros_like(xi, dtype=torch.float32)
            xi = torch.cat((xi_mask, xi), -1)
